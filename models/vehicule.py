...
class TransportVehicule(models.Model):
    ...
    _order = 'imatriculation'
     
    def _vehicle_name_get_fnc(self):
        name=""
        for record in self:
            name =  record.imatriculation
            if record.model_id:
                name =name +'/'+ record.model_id.name
                if record.model_id.brand_id:
                     name =name + '/' +record.model_id.brand_id.name  
        return name
    ...
    def return_action_to_open(self):
        ...
        if self.env.context.get('xml_id'):
            ...
            res['context'].update({'default_vehicule_id': ids[0]})
            res['domain'] = [('vehicule_id','=', ids[0])]
            return res
        return False
    ...
    def act_show_log_cost(self):
        ...
        ids = [record.id for record in self] 
        
        res['context'].update({
            'default_vehicule_id':ids[0],
    ...
    def _get_odometer(self):
        ...
        for record in self:
            ids = self.pool.get('fleet.vehicle.odometer').search(self.env.cr, self.env.uid, [('vehicule_id', '=', record.id)], limit=1, order='value desc')
            if len(ids) > 0:
    ...
    def _set_odometer(self):
        if value:
            date = datetime.now()
            data = {'value': value, 'date': date, 'vehicule_id': id}
    ...
    def _search_get_overdue_contract_reminder(self):
        ...
        for field, operator, value in self.env.args:
            ...
            datetime.now()
            cr.execute('select cost.vehicule_id, count(contract.id) as contract_number FROM fleet_vehicle_cost cost left join fleet_vehicle_log_contract contract on contract.cost_id = cost.id WHERE contract.expiration_date is not null AND contract.expiration_date < %s AND contract.state IN (\'open\', \'toclose\') GROUP BY cost.vehicule_id', (today,))
            ...
            for element in record.log_contracts:
                if element.state in ('open', 'toclose') and element.expiration_date:
                    ...
                    if overdue or due_soon:
                        records = self.env['fleet.vehicle.log.contract'].search([('vehicule_id', '=', record.id), ('state', 'in', ('open', 'toclose'))],limit=1, order='expiration_date asc')
                        for model in records :
                            records = model.cost_subtype_id.name
    ...
    def _count_all(self):
        ...
        return {
            vehicule_id: {
                'odometer_count': Odometer.search_count(self.env.cr, self.env.uid, [('vehicule_id', '=', vehicule_id)], context=self.env.context),
                'fuel_logs_count': LogFuel.search_count(self.env.cr, self.env.uid, [('vehicule_id', '=', vehicule_id)], context=self.env.context),
                'service_count': LogService.search_count(self.env.cr, self.env.uid, [('vehicule_id', '=', vehicule_id)], context=self.env.context),
                'contract_count': LogContract.search_count(self.env.cr, self.env.uid, [('vehicule_id', '=', vehicule_id)], context=self.env.context),
                'cost_count': Cost.search_count(self.env.cr, self.env.uid, [('vehicule_id', '=', vehicule_id), ('parent_id', '=', False)], context=self.env.context)
            }
            for vehicule_id in ids
        }
    @api.multi
    def name_get(self):
        result = []
    ...
    tag_ids = fields.Many2many('fleet.vehicle.tag', 'transport_vehicle_vehicule_tag_rel', 'vehicule_tag_id', 'tag_id', string='Tags')
    ...
    transmission = fields.Selection([
            ('manual', 'Manual'),
            ('automatic', 'Automatic'),
        ], string='Transmission', index=True, readonly=True, default='manual', copy=False,
        states={'draft': [('readonly', False)]},
    ...
    co2 = fields.Float(
        string='Emission de CO2',
        required=False,
        readonly=True,
        select=True,
        states={'draft': [('readonly', False)]},
        help='Emission de CO2'
    )
    ...
    cost_count = fields.Integer(string='Costs', compute='_count_all', store=True, readonly=True, track_visibility='always')
    contract_count = fields.Integer(string='Contracts',compute='_count_all', store=True, readonly=True,  track_visibility='always')
    service_count = fields.Integer(string='Services',compute='_count_all', store=True, readonly=True,  track_visibility='always')
    fuel_logs_count = fields.Integer(string='Fuel Logs',compute='_count_all', store=True, readonly=True,  track_visibility='always')
    odometer_count = fields.Float(string='Odometer',compute='_count_all', store=True, readonly=True,  track_visibility='always')
    odometer = fields.Integer(string='Last Odometer', compute='_get_odometer', store=True, readonly=True,inverse='_set_odometer', track_visibility='always')
    
    contract_renewal_due_soon = fields.Integer(string='Has Contracts to renew',compute='_get_contract_reminder_fnc', store=True, readonly=True,  track_visibility='always')
    contract_renewal_overdue = fields.Integer(string='Has Contracts Overdue', compute='_get_contract_reminder_fnc', store=True, readonly=True,track_visibility='always')
    contract_renewal_name = fields.Integer(string='Name of contract to renew soon', compute='_get_contract_reminder_fnc', store=True, readonly=True, track_visibility='always')
    contract_renewal_total = fields.Integer(string='Total of contracts due or overdue minus one',compute='_get_contract_reminder_fnc', store=True, readonly=True,  track_visibility='always')
    
    service_count = fields.Integer(string='Total of contracts due or overdue minus one', compute='_get_contract_reminder_fnc',  store=True, readonly=True,track_visibility='always')
    ...
    count_feuilleroute_ids = fields.Integer(
        string='Feuilles de route',
        compute='_compute_count_feuilleroute_ids',
    )
    count_piece_ids = fields.Integer(
        string='Pièces de véhicule ',
        compute='_compute_count_piece_ids',
    )
     
 
    _sql_constraints = [
            ('imatriculation_uniq', 'unique (imatriculation)', "Cet imatriculation existe déja !"),
    ]
    @api.multi
    ...
    @api.multi
    def _compute_count_feuilleroute_ids(self):
        for record in self:
            record.count_feuilleroute_ids = len(record.feuilleroute_ids)
    @api.multi
    def _compute_count_piece_ids(self):
        for record in self:
            record.count_piece_ids = len(record.piece_ids)
    ...
    def on_change_model(self): 
        for vehicule in self:
            if vehicule.model_id:
                records = self.env['fleet.vehicle.model'].search([('id', '=', vehicule.model_id.id)])
                for model in records :
                    vehicule.image_medium = model.image               
    ...
    def create(self,values):
        ...
        if 'state' in values:
            values['state'] = 'draft'
        vehicule_id = super(TransportVehicule, self.with_context(mail_create_nolog=True)).create(values)
        #vehicle = self.browse(vehicule_id)
        #self.message_post(self.env.cr, self.env.uid, [vehicule_id], body=_('%s %s has been added to the fleet!') % (vehicle.model_id.name,vehicle.imatriculation), context=context)
        return vehicule_id
    ...
    def write(self,vals):
        ...
        vehicule_id = super(TransportVehicule,self.with_context(mail_create_nolog=True)).write(vals)
        return True    
