...
class TransportChauffeur(models.Model):
    ...
    employee_id = fields.Many2one(
        ...
        index=True,
        readonly=False,
        help='L\'employé rattaché Ã  ce chauffeur',
    )
    ...
    def create(self,values):
        if 'state' in values:
            values['state'] = 'draft'
        record = super(TransportChauffeur, self.with_context(mail_create_nolog=True)).create(values)
        return record 

    @api.multi
    def write(self,values):
        record = super(TransportChauffeur, self.with_context(mail_create_nolog=True)).write(values)
        return record
    ...
    def unlink(self):
        for record in self:
            if record.state == 'draft' or record.state=='cancel':
                #super(TransportChauffeur, self).unlink()
